...
class SocketClient:
    def __init__(self, host="localhost", port=5001):
        ...
        while not connected:
            try:
                ...
                self.client.connect((host, port))  # 서버에 연결
                connected = True
    ...
    def send_data(self, cmd):
        """ 시선 좌표 데이터를 서버로 전송하는 함수 """
        try:
            data = cmd.to_bytes(1, byteorder='big')
            self.client.send(data)  # 데이터를 서버로 전송
        except:
            print("❌ 서버 연결 끊김")
            self.client.close()
            connected = False
            while not connected:
                try:
                    self.client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    self.client.connect(("localhost", 5001))  # 서버 재연결
                    connected = True
                    print("✅ 서버 재연결됨")
                except:
                    connected = False
